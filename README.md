...
本工具能够可视化地从海量（GB级）的 OSM PBF 全量地图数据中，高效裁剪出目标矩形区域。内置了强大的国内地图坐标系（GCJ-02 / BD-09）自动纠偏算法，完美解决了使用中国底图选点导致的路网偏移痛点，确保输出的路网绝对对齐 **WGS-84** 国际标准。

---

##Features

* **交互式地图选区**：内置 OSM 原生、高德地图、Esri 卫星图三种底图。
* **坐标系智能纠偏**：在使用高德等国内底图选点时，程序会自动将带偏移的 GCJ-02 坐标逆向转换为纯净的 WGS-84 坐标，并在前端双向对照显示，确保生成的寻路网格分毫不差。
...
国内地图（如高德、百度）为了地理信息安全，对真实经纬度进行了非线性加密。本工具内置了完整的正向与逆向数学解析模型，确保切割坐标的绝对精确。

### 1. WGS-84 $\mapsto$ GCJ-02 (正向加密)
...
### 2. GCJ-02 $\mapsto$ WGS-84 (工具核心：数值逼近解密)
因加密映射包含超越方程，解析逆映射严格不存在。本工程代码采用了高效的**一阶线性迭代逼近算法**：
假设在极小邻域内梯度趋于零（$\Delta T(P_W) \approx \Delta T(P_G)$），构造逼近迭代式：
$$P_{WGS} \approx 2P_{GCJ} - T_{Forward}(P_{GCJ})$$
该算法的绝对误差 $\epsilon < 10^{-5}$ 度（约 0.5 米内），完美满足商业寻路引擎（如 OSRM）的拓扑精度需求。

### 3. GCJ-02 $\mapsto$ BD-09 (百度极坐标微扰)
